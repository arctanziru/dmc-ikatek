...
<div class="p-6 space-y-4">
    ...
    <div class="flex items-center space-x-4 ">
        <x-bladewind::input type="text" wire:model.live.debounce.500ms="search" placeholder="Search users..."
            class="border h-full" />
            ...
            <tbody>
                @foreach($users as $user)
                    <tr>
                        ...
                        <td>{{ $user->role }}</td>
                        <td>
                            <x-bladewind::button wire:click="redirectToEdit({{ $user->id }})" size="small" color="primary"
                                icon="pencil-square">
                                Edit
                            </x-bladewind::button>
                            @if ($user->id !== auth()->user()->id)
                                <x-bladewind::button wire:click="deleteUser({{ $user->id }})"
