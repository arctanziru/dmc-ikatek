<main class="p-4">
    <form wire:submit.prevent="save" class="flex flex-col">
        ...
        <x-bladewind::input type="password" wire:model="form.password" label="Password" placeholder="Enter password" />
        @error('form.password') <span class="text-red-600 -mt-4 mb-4">{{ $message }}</span> @enderror

        <x-button type="submit" class="self-end">Create User</x-button>
    </form>
</main>