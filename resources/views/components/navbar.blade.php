...
<nav id="navbar"
    class="w-screen p-[24px_48px] items-center flex justify-between z-[100] top-0 margin-0 {{ $navbarVariantClass }} {{ $class }}">
    ...
    <div class="flex gap-5 items-center justify-center">
        <div class="flex gap-6">
            @foreach ($links as $link)
                <a href="{{ $link['url'] }}"
                    class="text-[14px] transition-[200ms] hover:transition-[200ms] hover:text-primary">{{ $link['name'] }}</a>
            @endforeach
        </div>
